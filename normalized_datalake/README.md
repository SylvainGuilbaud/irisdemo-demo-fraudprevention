...
from VS Code connected in fraudprevention-datalake
import & compile package : normalized_datalake/src/cls/cubes 
import & compile class : normalized_datalake/src/cls/IRISDemo/CheckingTrans.cls 
from System Management Portal : 
imports the pivots and dashboard definitions : 
http://localhost:9094/csp/app/_DeepSee.UI.FolderManager.cls?$ERROR=1&$NAMESPACE=APP : >> normalized_datalake/analytics/2pivots_2dashboards.xml
build the cube :
http://localhost:9094/csp/app/_DeepSee.UI.Architect.zen?$NAMESPACE=APP&CUBE=transactions.cube : >> build 
Run Dashboard :
http://localhost:9094/csp/app/_DeepSee.UserPortal.DashboardViewer.zen?DASHBOARD=fraudPrevention/today.dashboard&EMBED=1
import the postman file in Postman : normalized_datalake/postman/Fraud Prevention.postman_collection.json
run the http://localhost:9092/csp/appint/rest/whitelist/ with "FromAccountNumber":"C1822295676" to add the customer in the white list
